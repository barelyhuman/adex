// Dev-mode Vite plugin for the Node.js adapter.
// This file is only ever loaded in a Node.js/Vite context (build tooling),
// so Node-only APIs (Buffer, node:path, adex/http) are safe to use here.

import { nodeRequestToFetch, fetchResponseToNode } from 'adex/http'
import { resolve } from 'node:path'

/**
 * Creates the Vite dev server plugin for the Node.js adapter.
 *
 * @param {{ islands: boolean }} options
 * @returns {import('vite').Plugin}
 */
export function createNodeDevServerPlugin({ islands = false } = {}) {
